//자료구조 한옥영 교수님 8주차 LAB04 Deque ADT 구현
//최종 작성일: 2023.06.09(금)
//2019313550 박병현
#include <stdio.h>
#include <stdlib.h>
...
void print(Deque* deque) {
    ...
    // 원래 덱의 데이터를 출력하고 임시 덱에 추가
    while (!is_empty(deque)) {
        char data = delete_front(deque);
        printf("%c ", data);
        add_rear(&temp_deque, data);
    }
    
    printf("\n");

    // 임시 덱의 데이터를 다시 원래 덱에 추가
    while (!is_empty(&temp_deque)) {
        char data = delete_front(&temp_deque);
        add_rear(deque, data);
    }
}
char get_front(Deque* deque) {//덱의 front에 저장된 데이터를 보여주는 함수
