...
void move(char arr[], int index){
    ...
    // 입력된 index가 유효한 범위 내에 있는지 확인
    if (index < 0 || index >= size) {
        printf("이동할 위치가 유효한 범위를 벗어났습니다.\n");
        return; //함수를 종료하여 이동을 수행하지 않음
    }
    
    // MP를 배열의 첫 번째 요소에 대해 수행하거나, MN을 배열의 마지막 요소에 대해 수행한 경우 처리
    if ((index == lct - 1 && lct == 0) || (index == lct + 1 && lct == size - 1)) {
        printf("현재 위치에서 더 이상 이동할 수 없습니다.\n");
        return; //함수를 종료하여 이동을 수행하지 않음
    }
