...
void add_edge(GRAPH* my_graph, char data1, char data2, int weight);
...
void show();
void find_max_degree(GRAPH* my_graph);//나만의 기능2
void find_path_way(GRAPH* my_graph, VERTEX* data1, VERTEX* data2);//나만의 기능3
void find_max_eight_edge(GRAPH* my_graph);//나만의 기능4
void print_edge_weight(GRAPH* my_graph, char data1, char data2);//나만의 기능5
	...
	while (1) {
		...
		for (int i = 0; enter[i] != '\0'; i++) {
			if (enter[i] == 'G') {
                ...
                int data2 = enter[i+4];
                i+=6;
				add_edge(my_graph,data1,data2,CharToNum(enter,&i));
			}
			else if (enter[i] == 'L') {
                continue;
            }
                ...
                find_max_eight_edge(my_graph);
                goto A;
            }
            else if (enter[i]=='W'){
                print_edge_weight(my_graph, enter[i+2], enter[i+4]);
                i+=5;
                goto A;
            }
			else if (enter[i] == '*') {
...
void show(){
    ...
    printf("E(값1,값2, 가중치): 두 개의 값을 간선으로 연결하고 입력하신 가중치를 부여합니다.\n");
    ...
    printf("~(값1,값2): 값1에서 값2로 가는 최단 경로를 출력합니다.\n"); //나만의 기능1
    printf("M: 차수가 가장 큰 정점의 값과 그 차수를 출력합니다.\n");//나만의 기능2
    printf("?(값1): 해당 값이 그래프에 존재하면 True, 존재하지 않으면 False를 출력합니다.\n");//나만의 기능3
    printf("B: 가중치가 가장 큰 간선이 연결하는 정점과 그 가중치를 출력합니다.\n");//나만의 기능4
    printf("W(값1,값2): 두 정점을 연결한 간선의 가중치를 출력합니다.n");//나만의 기능5
    printf("*: 프로그램을 종료합니다.\n");
}
void find_path_way(GRAPH* my_graph, VERTEX* startVertex, VERTEX* endVertex) {
    int distances[100];
    int visited[100] = { 0 };
    char previousVertex[100] = { '\0' };
    char path[100] = { '\0' };
    int exists[100] = { 0 };
    ...
    distances[startVertex->data - 'A'] = 0;

    VERTEX* tempVertex = my_graph->head;
    while (tempVertex) {
        exists[tempVertex->data - 'A'] = 1;
    ...
    while (1) {
        ...
        int currentVertex;
        for (int i = 0; i < 100; i++) {
            if (exists[i] == 1 && distances[i] < min && visited[i] == 0) {
                min = distances[i];
                currentVertex = i;
            }
        }
        ...
        tempVertex = search_vertex(my_graph, currentVertex + 'A');
        EDGE* tempEdge = tempVertex->connect;

        while (tempEdge) {
            if (visited[tempEdge->vertex->data - 'A'] == 0 && (min + tempEdge->weight) < distances[tempEdge->vertex->data - 'A']) {
                distances[tempEdge->vertex->data - 'A'] = (min + tempEdge->weight);
                previousVertex[tempEdge->vertex->data - 'A'] = (currentVertex + 'A');
            }

            tempEdge = tempEdge->more;
        }

        if ((currentVertex + 'A') == endVertex->data) {
            break;
        }
    }

    char endCharacter = endVertex->data;
    char startCharacter = startVertex->data;
    
    int i = 0;
    while (endCharacter != startCharacter) {
        path[i] = previousVertex[endCharacter - 'A'];
        endCharacter = previousVertex[endCharacter - 'A'];
        i++;
    }
    ...
    printf("%c\n", endVertex->data);
    printf("가중치 합: %d\n", distances[endVertex->data - 'A']);
}
void find_max_degree(GRAPH* my_graph) {
...
void find_max_eight_edge(GRAPH* my_graph) {
    int maxWeight = -1;
    char start = '\0', end = '\0';
    ...
    while(tempVertex) {
        EDGE* tempEdge = tempVertex->connect;
        while(tempEdge) {
            if(tempEdge->weight > maxWeight) {
                maxWeight = tempEdge->weight;
                start = tempVertex->data;
                end = tempEdge->vertex->data;
            }
            tempEdge = tempEdge->more;
        }
        tempVertex = tempVertex->next;
    ...
    if(maxWeight == -1) {
        printf("그래프에 간선이 존재하지 않습니다.\n");
    } else {
        printf("%c 와 %c를 연결하는 간선의 가중치가 %d으로 가장 큽니다.\n", start, end, maxWeight);
    }
}
void print_edge_weight(GRAPH* my_graph, char data1, char data2) {
    VERTEX* tempVertex = my_graph->head;
    while (tempVertex) {
        if (tempVertex->data == data1) {
            EDGE* tempEdge = tempVertex->connect;
            while (tempEdge) {
                if (tempEdge->vertex->data == data2) {
                    printf("%c와 %c 사이의 가중치는 %d입니다.\n", data1, data2, tempEdge->weight);
                    return;
                }
                tempEdge = tempEdge->more;
            }
